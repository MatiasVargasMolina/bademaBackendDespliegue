...
public interface PedidoRepository extends JpaRepository<PedidoEntity, Long> {
    // @Query("SELECT new
    // usach.pingeso.badema.dtos.obra.PedidosATrabajarListDTO(p.id, p.nombre,
    // p.estado, p.fechaPedido) " +
    // "FROM PedidoEntity p WHERE p.responsable.obra.id = :idObra AND p.estado =
    // :estado")
    // List<PedidosATrabajarListDTO> findByEstadoObra(Long idObra, int estado);
    //
    // @Query("SELECT new
    // usach.pingeso.badema.dtos.obra.PedidosATrabajarListDTO(p.id, p.nombre,
    // p.estado, p.fechaPedido) " +
    // "FROM PedidoEntity p WHERE p.responsable.obra.id = :idObra AND p.fechaPedido
    // = :fechaPedido")
    // List<PedidosATrabajarListDTO> findByFechaPedidoObra(Long idObra, LocalDate
    // fechaPedido);
